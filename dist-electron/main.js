...
__export(schema_exports, {
  ...
  serviceChecklistsRelations: () => serviceChecklistsRelations,
  services: () => services,
  servicesRelations: () => servicesRelations
});
var centerProfiles = sqliteTable("center_profiles", {
...
var faqEntries = sqliteTable("faq_entries", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var serviceChecklists = sqliteTable("service_checklists", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var serviceChecklistsRelations = relations(
  serviceChecklists,
  ({ one }) => ({
    service: one(services, {
...
var messageTemplates = sqliteTable("message_templates", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  name: text("name").notNull(),
...
var agentLogs = sqliteTable("agent_logs", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
function initDatabase(dbPath2) {
  sqlite = new import_better_sqlite32.default(dbPath2);
...
// src/shared/types.ts
var PaymentMode = external_exports.enum(["Cash", "UPI", "Card", "Other"]);
var InvoiceStatus = external_exports.enum(["PAID", "PENDING", "CANCELLED"]);
var LeadStatus = external_exports.enum(["NEW", "CONTACTED", "CONVERTED", "CLOSED"]);
var MessageChannel = external_exports.enum(["whatsapp", "sms", "both"]);
var centerProfileSchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  centerName: external_exports.string().default(""),
  address: external_exports.string().default(""),
  mobile: external_exports.string().default(""),
  email: external_exports.string().email().or(external_exports.literal("")).default(""),
  udyamNumber: external_exports.string().default(""),
  logoPath: external_exports.string().nullable().default(null),
  upiQrPath: external_exports.string().nullable().default(null),
  upiId: external_exports.string().nullable().default(null),
  invoicePrefix: external_exports.string().default("INV-"),
  invoiceNumber: external_exports.number().int().nonnegative().default(0),
  theme: external_exports.string().default("light"),
  defaultTaxRate: external_exports.number().nonnegative().default(0),
  defaultPaymentMode: PaymentMode.default("Cash"),
  lastBackupDate: external_exports.coerce.date().nullable().default(null),
  pinHash: external_exports.string().nullable().default(null),
  operatingHours: external_exports.string().default(""),
  centerDescription: external_exports.string().default("")
});
var serviceSchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  name: external_exports.string().min(1, "Service name is required"),
  category: external_exports.string().default("Other"),
  defaultPrice: external_exports.number().nonnegative().default(0),
  taxRate: external_exports.number().nonnegative().max(100).default(0),
  isActive: external_exports.boolean().default(true),
  isBookmarked: external_exports.boolean().default(false),
  keywords: external_exports.string().default("")
});
var customerSchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  name: external_exports.string().min(1, "Customer name is required"),
  mobile: external_exports.string().default(""),
  remarks: external_exports.string().nullable().default(null),
  email: external_exports.string().email().or(external_exports.literal("")).default(""),
  address: external_exports.string().default(""),
  tags: external_exports.string().default("")
});
var invoiceItemSchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  invoiceId: external_exports.number().int().positive().optional(),
  serviceId: external_exports.number().int().positive(),
  description: external_exports.string().min(1),
  qty: external_exports.number().int().positive(),
  rate: external_exports.number().nonnegative(),
  taxRate: external_exports.number().nonnegative().max(100).default(0),
  lineTotal: external_exports.number().nonnegative()
});
var invoiceSchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  invoiceNo: external_exports.string(),
  createdAt: external_exports.coerce.date().optional(),
  customerId: external_exports.number().int().positive(),
  subtotal: external_exports.number().nonnegative(),
  taxTotal: external_exports.number().nonnegative().default(0),
  discount: external_exports.number().nonnegative().default(0),
  total: external_exports.number().nonnegative(),
  paymentMode: PaymentMode.default("Cash"),
  status: InvoiceStatus.default("PAID"),
  notes: external_exports.string().nullable().default(null),
  customerNotes: external_exports.string().nullable().default(null),
  printedAt: external_exports.coerce.date().nullable().default(null),
  items: external_exports.array(invoiceItemSchema).optional()
});
var createInvoiceSchema = external_exports.object({
  customerId: external_exports.number().int().positive().optional(),
  newCustomer: external_exports.object({
  ...
  customerNotes: external_exports.string().optional()
});
var faqEntrySchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  ...
  sortOrder: external_exports.number().int().nonnegative().default(0),
  createdAt: external_exports.coerce.date().optional(),
  updatedAt: external_exports.coerce.date().optional()
});
var serviceChecklistSchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  serviceId: external_exports.number().int().positive(),
  documentName: external_exports.string().min(1),
  isRequired: external_exports.boolean().default(true),
...
var leadSchema = external_exports.object({
  ...
  mobile: external_exports.string().default(""),
  email: external_exports.string().email().or(external_exports.literal("")).default(""),
  serviceInterest: external_exports.string().default(""),
  source: external_exports.string().default("manual"),
  status: LeadStatus.default("NEW"),
  notes: external_exports.string().nullable().default(null),
  createdAt: external_exports.coerce.date().optional(),
  convertedCustomerId: external_exports.number().int().positive().nullable().default(null)
});
var messageTemplateSchema = external_exports.object({
  id: external_exports.number().int().positive().optional(),
  name: external_exports.string().min(1),
...
function checkPort(host, port = 9100, timeout = 500) {
...
async function scanForPrinters() {
  ...
  return hosts.filter((_, i) => results[i]).map((h) => ({ ip: h, port: 9100, interface: `tcp://${h}:9100` }));
}

// src/main/index.ts
var userDataPath = import_electron.app.getPath("userData");
var dbPath = import_path.default.join(userDataPath, "csc_billing.db");
var uploadsPath = import_path.default.join(userDataPath, "uploads");
process.env.DATABASE_URL = `file:${dbPath}`;
process.env.USER_DATA_PATH = userDataPath;
var mainWindow = null;
var nextProcess = null;
var digestShownToday = "";
...
function parseArg(schema, arg, label) {
  const result = schema.safeParse(arg);
  if (!result.success) {
    throw new Error(
      `[${label}] invalid input: ${result.error.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join("; ")}`
    );
  }
  return result.data;
}
...
var DEFAULT_BOOKMARKS = /* @__PURE__ */ new Set([
  "Aadhaar Update",
  "PAN Application",
  "Xerox / Print (B&W)",
  "Form Filling",
...
function bootstrapDatabase() {
  if (!import_fs.default.existsSync(import_path.default.dirname(dbPath))) {
    import_fs.default.mkdirSync(import_path.default.dirname(dbPath), { recursive: true });
  }
  if (!import_fs.default.existsSync(uploadsPath)) {
    import_fs.default.mkdirSync(uploadsPath, { recursive: true });
  }
  ...
  `);
  const cols = sqlite2.prepare("PRAGMA table_info(services)").all();
  if (!cols.find((c) => c.name === "is_bookmarked")) {
    sqlite2.exec(
      "ALTER TABLE services ADD COLUMN is_bookmarked INTEGER NOT NULL DEFAULT 0"
    );
  }
  const profileCount = sqlite2.prepare("SELECT COUNT(*) as count FROM center_profiles").get();
  if (profileCount.count === 0) {
    db2.insert(centerProfiles).values({}).run();
  }
  const serviceCount = sqlite2.prepare("SELECT COUNT(*) as count FROM services").get();
...
function startStandaloneServer() {
  const serverPath = import_path.default.join(
  ...
  nextProcess = (0, import_child_process.spawn)(process.execPath, [serverPath], {
    env: {
      ...process.env,
  ...
  nextProcess.stderr?.on(
    "data",
    (d) => console.error(`[next:err] ${d}`)
  );
  setTimeout(tryLoad, 6e3);
}
function registerIpcHandlers() {
  const db2 = getDb();
  safeHandle(IPC.APP_VERSION, () => import_electron.app.getVersion());
  safeHandle(IPC.APP_DB_PATH, () => dbPath);
  safeHandle(
    IPC.CENTER_GET,
    () => db2.select().from(centerProfiles).get()
  );
  safeHandle(IPC.CENTER_UPDATE, (_e, raw) => {
    ...
    db2.update(centerProfiles).set(data).where(eq(centerProfiles.id, 1)).run();
    return db2.select().from(centerProfiles).get();
  });
  safeHandle(
    IPC.CUSTOMERS_LIST,
    () => db2.select().from(customers).all()
  );
  safeHandle(
    IPC.CUSTOMERS_GET,
    (_e, id) => db2.select().from(customers).where(eq(customers.id, external_exports.number().int().parse(id))).get()
  );
  safeHandle(IPC.CUSTOMERS_CREATE, (_e, raw) => {
    ...
    return db2.insert(customers).values(data).returning().get();
  });
  safeHandle(IPC.CUSTOMERS_UPDATE, (_e, id, raw) => {
    const _id = external_exports.number().int().parse(id);
    const data = parseArg(
      customerSchema.partial(),
      raw,
      "customers:update"
    );
    return db2.update(customers).set(data).where(eq(customers.id, _id)).returning().get();
  });
  safeHandle(IPC.CUSTOMERS_DELETE, (_e, id) => {
    const _id = external_exports.number().int().parse(id);
    db2.delete(customers).where(eq(customers.id, _id)).run();
    return { success: true };
  });
  safeHandle(IPC.CUSTOMERS_SEARCH, (_e, query) => {
    const q = external_exports.string().parse(query);
    const pattern = `%${q}%`;
    return db2.select().from(customers).where(
      sql`${customers.name} LIKE ${pattern} OR ${customers.mobile} LIKE ${pattern}`
    ).all();
  });
  safeHandle(
    IPC.SERVICES_LIST,
    () => db2.select().from(services).all()
  );
  safeHandle(
    IPC.SERVICES_GET,
    (_e, id) => db2.select().from(services).where(eq(services.id, external_exports.number().int().parse(id))).get()
  );
  safeHandle(IPC.SERVICES_CREATE, (_e, raw) => {
    ...
    return db2.insert(services).values(data).returning().get();
  });
  safeHandle(IPC.SERVICES_UPDATE, (_e, id, raw) => {
    const _id = external_exports.number().int().parse(id);
    const data = parseArg(serviceSchema.partial(), raw, "services:update");
    return db2.update(services).set(data).where(eq(services.id, _id)).returning().get();
  });
  safeHandle(IPC.SERVICES_DELETE, (_e, id) => {
    const _id = external_exports.number().int().parse(id);
    db2.delete(services).where(eq(services.id, _id)).run();
    return { success: true };
  });
  safeHandle(IPC.SERVICES_TOGGLE_BOOKMARK, (_e, id) => {
    const _id = external_exports.number().int().parse(id);
    const svc = db2.select().from(services).where(eq(services.id, _id)).get();
    if (!svc)
      throw new Error("Service not found");
    return db2.update(services).set({ isBookmarked: !svc.isBookmarked }).where(eq(services.id, _id)).returning().get();
  });
  safeHandle(
    IPC.SERVICES_GET_BOOKMARKED,
    () => db2.select().from(services).where(
      and(
        eq(services.isBookmarked, true),
  ...
  safeHandle(
    IPC.INVOICES_GET,
    (_e, id) => db2.query.invoices.findFirst({
      where: eq(invoices.id, external_exports.number().int().parse(id)),
  ...
  safeHandle(IPC.INVOICES_CREATE, (_e, raw) => {
    ...
    const sqlite2 = getSqlite();
    return sqlite2.transaction(() => {
      let customerId = data.customerId;
      if (!customerId && data.newCustomer) {
        const created = db2.insert(customers).values({
      ...
      const profile = db2.select().from(centerProfiles).where(eq(centerProfiles.id, 1)).get();
      ...
      for (const it of items) {
        db2.insert(invoiceItems).values({
      ...
      db2.update(centerProfiles).set({ invoiceNumber: seq }).where(eq(centerProfiles.id, 1)).run();
  ...
  safeHandle(IPC.INVOICES_UPDATE_STATUS, (_e, id, status) => {
    ...
    return db2.update(invoices).set({ status: _status }).where(eq(invoices.id, _id)).returning().get();
  });
  safeHandle(IPC.INVOICES_DELETE, (_e, id) => {
    const _id = external_exports.number().int().parse(id);
    const sqlite2 = getSqlite();
    sqlite2.transaction(() => {
      db2.delete(invoiceItems).where(eq(invoiceItems.invoiceId, _id)).run();
      db2.delete(invoices).where(eq(invoices.id, _id)).run();
    })();
    return { success: true };
  });
  ...
  safeHandle(IPC.REPORTS_DAILY, (_e, dateStr) => {
    const _date = external_exports.string().parse(dateStr);
    const start = new Date(_date);
    start.setHours(0, 0, 0, 0);
    const end = new Date(_date);
    end.setHours(23, 59, 59, 999);
    return db2.query.invoices.findMany({
      where: and(
  ...
  safeHandle(IPC.REPORTS_RANGE, (_e, startStr, endStr) => {
    const s = external_exports.string().parse(startStr);
    const e = external_exports.string().parse(endStr);
  ...
  safeHandle(IPC.PRINTER_SET_CONFIG, (_e, cfg) => {
    const parsed = parseArg(
      external_exports.object({
        ...
        printUpiQr: external_exports.boolean().optional()
      }).strict(),
      cfg,
      "printer:set-config"
    );
  ...
  safeHandle(IPC.BACKUP_EXPORT, () => {
    const backupDir = import_path.default.join(import_electron.app.getPath("documents"), "CSC-Backups");
    if (!import_fs.default.existsSync(backupDir))
      import_fs.default.mkdirSync(backupDir, { recursive: true });
    const stamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-").split("T")[0];
    const out = import_path.default.join(backupDir, `csc_billing_${stamp}.db`);
    import_fs.default.copyFileSync(dbPath, out);
  ...
  safeHandle(IPC.BACKUP_IMPORT, async () => {
    ...
    closeDatabase();
    import_fs.default.copyFileSync(result.filePaths[0], dbPath);
    initDatabase(dbPath);
...
async function performAutoBackup() {
  ...
  if (!import_fs.default.existsSync(backupDir))
    import_fs.default.mkdirSync(backupDir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-").split("T")[0];
  const out = import_path.default.join(backupDir, `csc_billing_auto_${stamp}.db`);
  if (import_fs.default.existsSync(out))
    return;
  import_fs.default.copyFileSync(dbPath, out);
  const files = import_fs.default.readdirSync(backupDir).filter((f) => f.startsWith("csc_billing_auto_") && f.endsWith(".db")).sort().reverse();
  for (let i = 7; i < files.length; i++) {
    import_fs.default.unlinkSync(import_path.default.join(backupDir, files[i]));
  }
}
  ...
  setInterval(
    () => {
      ...
      try {
        ...
        const invoices2 = db2.query.invoices.findMany({
          where: and(
            gte(invoices.createdAt, start),
            lte(invoices.createdAt, end)
          )
        }).sync();
        const total = invoices2.reduce(
          (s, i) => s + (i.total || 0),
          0
        );
        if (import_electron.Notification.isSupported()) {
          new import_electron.Notification({
            ...
            body: `Today: ${invoices2.length} invoices, Total: \u20B9${total.toFixed(0)}`
          }).show();
        }
      } catch (err) {
...
import_electron.app.whenReady().then(() => {
  ...
  createWindow();
  scheduleDailyDigest();
  if (import_electron.app.isPackaged && process.env.UPDATE_FEED) {
    import_electron_updater.autoUpdater.checkForUpdatesAndNotify().catch((e) => console.error("[auto-update]", e));
  }
